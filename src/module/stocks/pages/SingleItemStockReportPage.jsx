...
const SingleItemStockReportPage = () => {
  ...
  const formatClosingBalanceText = (pcs, sqr) => {
    ...
    const pcsFormatted = Number(p.toFixed(2));
    const sqftFormatted = Number(s.toFixed(2));

    const pcsText = p === 0 ? "Zero Pieces" : `${pcsFormatted} Pieces`;
    const sqftText = s === 0 ? "Zero SQFT" : `${sqftFormatted} SQFT`;
