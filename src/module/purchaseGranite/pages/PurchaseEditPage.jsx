...
const PurchaseEditPage = () => {
    ...
    if (purchaseByid) {
      ...
      if (pSub && pSub.length > 0) {
        const mappedData = pSub.map((sub) => ({
          ...
          purchase_sub_qnty: sub.purchase_sub_qnty?.toString() || sub.purchase_sub_pcs?.toString() || "",
    ...
    try {
      const formattedItemEntries = itemEntries.map((entry, index) => {
        return {
          ...
          purchase_sub_qnty: entry.purchase_sub_qnty || "0",
          purchase_sub_qnty_sqr: entry.purchase_sub_qnty_sqr || "0",
          purchase_sub_rate: entry.purchase_sub_rate || "0",
          purchase_sub_amount: entry.purchase_sub_amount || "0",
        };
      });
  ...
  if (isPurchaseLoading || isProductLoading || isInitialLoading) {
    return (
      <Page>
